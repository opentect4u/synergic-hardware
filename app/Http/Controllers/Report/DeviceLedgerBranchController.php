...
class DeviceLedgerBranchController extends Controller
{
    ...
    public function Show(Request $request)
    {
        ...
        if ($from_dt!='' && $to_dt!='' && $device_desc!='') {
            ...
            $opening=TdOpening::where('mc_type',$device_desc)
                ->where('date',$finalcial_date)
                ->where('serv_ctr',$srv_ctr)
                ->get();
            // return $opening;
            $alldatas=DB::table('td_device_trans')
                ...
                ->where('td_device_trans.serv_ctr',$srv_ctr)
        ...
        $service_centres=MdServiceCentre::get();
        return view('reports.device_ledger_branch',['from_dt'=>$from_dt,'to_dt'=>$to_dt,'srv_ctr'=>$srv_ctr,'device_desc'=>$device_desc,
        'machines'=>$machines,'service_centres'=>$service_centres,'opening'=>$opening,'alldatas'=>$alldatas,'device_name'=>$device_name]);
    }
}
